...
public class SendMessageUtils {
    ...
    static final String accessKeyId = "LTAIe8quflPoALvN";
...
//        return null;
    ...
    public static MessageRecord sendSms11(String userMobile, SMSEnum smsEnum, Map<String, String> map) {
        ...
        // 必填:待发送手机号
        request.setPhoneNumbers(userMobile);
        // 必填:短信签名-可在短信控制台中找到
        request.setSignName("ybhxjy");
        // 必填:短信模板-可在短信控制台中找到
        request.setTemplateCode(smsEnum.getCode());
        ...
        messageRecord.setMobile(userMobile);
        messageRecord.setUuid(UUID.randomUUID().toString());
        messageRecord.setCreateTime(LocalDateTime.now());
        messageRecord.setUpdateTime(LocalDateTime.now());
        if (sendSmsResponse != null) {
            messageRecord.setMessage(sendSmsResponse.getMessage());
            messageRecord.setStatus(1);
            messageRecord.setBizId(messageRecord.getBizId());
        } else {
            messageRecord.setStatus(0);
            messageRecord.setMessage(JSON.toJSONString(map));
        }
        return messageRecord;
    }
    
    public static QuerySendDetailsResponse querySendDetails(MessageRecord record) throws ClientException {
        // 可自助调整超时时间
        System.setProperty("sun.net.client.defaultConnectTimeout", "10000");
        System.setProperty("sun.net.client.defaultReadTimeout", "10000");
        // 初始化acsClient,暂不支持region化
        ...
        // 必填-号码
        request.setPhoneNumber(record.getMobile());
        // 可选-流水号
